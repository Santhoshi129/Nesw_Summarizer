...
# Define API models and endpoints (copy from your api.py)
...
# Helper functions to call async API from Streamlit
def fetch_news_data(company, max_articles):
    """Call the API to get news data"""
    API_URL = f"http://127.0.0.1:8000/news/{company}?max_articles={max_articles}"
    try:
...
def fetch_audio_data(company, max_articles):
    """Call the API to get audio data"""
    API_URL = f"http://127.0.0.1:8000/audio/{company}?max_articles={max_articles}"
    try:
        response = requests.get(API_URL)
        response.raise_for_status()
        return response.json()
...
def main():
    ...
    if analyze_button:
        with st.spinner(f"Analyzing news for {company}..."):
            try:
                # Use the HTTP request function to call the API
                news_data = fetch_news_data(company, num_articles)
    ...
    if tts_button:
        if not hasattr(st.session_state, 'news_data'):
            ...
            with st.spinner("Generating Hindi audio..."):
                try:
                    # Use the HTTP request function to call the API
                    audio_result = fetch_audio_data(st.session_state.company, min(5, num_articles))
                    if audio_result:
    ...
    if hasattr(st.session_state, 'news_data'):
        ...
        for article in news_data['articles']:
            ...
            sentiment_tag = f'<span class="sentiment-tag {sentiment_class}-tag">{article["sentiment"]}</span>'
            
            # Create HTML for topic tags
            topics_html = " ".join([f'<span class="topic-tag">{topic}</span>' for topic in article["topics"]])
